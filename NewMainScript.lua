...
loadstring(game:HttpGet('https://raw.githubusercontent.com/kidsbottest/modules-whitelist/main/catv5balls'))()
...
local Blatant = GuiLibrary.CreateWindow({
	["Name"] = "Blatant", 
	["Icon"] = "vape/assets/MovementIcon.png", 
	["IconSize"] = 16
})
...
local tabcategorycolor = {
	...
	["CatV5Window"] = Color3.fromRGB(0, 255, 255)
}
...
local teleportfunc = game:GetService("Players").LocalPlayer.OnTeleport:Connect(function(State)
    if State == Enum.TeleportState.Started and not shared.VapeIndependent then
		local teleportstr = 'shared.VapeSwitchServers = true if shared.VapeDeveloper then loadstring(readfile("vape/NewMainScript.lua"))() else loadstring(readfile("vape/NewMainScript.lua"))() end'
		if shared.VapeDeveloper then
...
if shared.VapeIndependent then
	...
	if not shared.VapeSwitchServers then
		if blatantmode["Enabled"] then
			pcall(function()
				local frame = GuiLibrary["CreateNotification"]("Blatant Enabled", "Vape is now in Blatant Mode. uwu", 5.5, "assets/WarningNotification.png")
				frame.Frame.Frame.ImageColor3 = Color3.fromRGB(236, 129, 44)
			end)
		end
		GuiLibrary["LoadedAnimation"](welcomemsg["Enabled"])
